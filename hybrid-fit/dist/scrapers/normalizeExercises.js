...
(function (ExerciseType) {
    ...
    ExerciseType["CONDITIONING"] = "conditioning";
 ...
 */
function determineExerciseType(ex) {
    ...
    if (ex.muscle || ex.details?.muscle) {
        if (ex.equipment?.some((e) => e?.toLowerCase().includes("stretch"))) {
 ...
 */
function normalizeSport(sport, ex) {
...
function normalizeEquipment(equipment) {
    ...
    return equipmentList.map(item => {
        ...
        if (mapped === '')
            return ''; // Filter out in the next step
...
function normalizeDifficulty(difficulty) {
    ...
    if (level.includes("begin"))
        return "beginner";
    if (level.includes("adv"))
        return "advanced";
    if (level.includes("int"))
        return "intermediate";
    return "intermediate";
}
function generateHybridId(ex, type, existingIds) {
    ...
    if (sourceUrl.includes("exrx.net")) {
        ...
        const hash = crypto_1.default.createHash("md5").update(sourceUrl).digest("hex").slice(0, 10);
        baseId = `exrx_${hash}_${type}`;
    }
    // 🔹 CASE 2: Others — use slug logic
    else {
        baseId = (0, slugify_1.default)(name, { lower: true, strict: true, replacement: "_" });
 ...
 */
function shortenMuscleName(muscle) {
    const muscleShortMap = {
 ...
 */
function generateUniqueName(ex, equipment) {
 ...
 */
function addMuscleSuffix(name, muscle) {
    if (!muscle)
        return name;
 ...
 */
function normalizeExercise(ex, ids) {
    ...
    const normalized = {
        ...
        details: {},
        durationMinutes: parseDuration(ex.durationMinutes ||
            ex.details?.duration ||
            ex.duration),
        tags: []
    };
    ...
    switch (sport) {
        ...
        case "strength_training":
            ...
            if (ex.variations && ex.variations.length > 0) {
                normalized.details.variations = ex.variations.map((v) => ({
                    name: v.name,
                    url: v.url,
    ...
    if (normalized.details.musclesWorked) {
        normalized.details.musclesWorked.forEach(m => tagSet.add(m.toLowerCase()));
    }
...
function deduplicateExercises(allExercises) {
 ...
 */
function ensureUniqueNames(exercises) {
 ...
 * Main entry point
 */
async function mergeAndNormalizeExercises() {
    ...
    await (0, saveDataAsJson_1.saveNormalizedExercises)(uniqueNamed);
    return uniqueNamed;
}
// Run the script
